...
def training_dl(
    ...
    for epoch in range(epochs):
        ...
        for idx, item in enumerate(trainloader):
            ...
            """
            # DataLoader에서 unpack
            input_ts, input_ts_mark, target_ts, target_ts_mark = item

            # 모델 forward
            outputs = model(input_ts, input_ts_mark, target_ts, target_ts_mark)
            # outputs: (B, pred_len, c_out)

            # Loss 계산
            # target_ts: (B, label_len+pred_len, c_out)      
            loss = criterion(outputs, target_ts[:, -pred_len:, :])
...
def test_dl(model, dataloader, criterion, accelerator: Accelerator, 
    ...
    with torch.no_grad():
        for idx, item in enumerate(dataloader):
            ...
            outputs, target_ts = accelerator.gather_for_metrics((outputs.contiguous(), target_ts[:, -pred_len:, :].contiguous()))

            losses_m.update(loss.item(), n=input_ts.size(0))
            outputs = outputs.detach().cpu().numpy()
            target_ts = target_ts[:, -pred_len:, :].detach().cpu().numpy()
    ...
    """
    if name == 'VALID':
        # validation은 loss만 반환
        results = {'loss': losses_m.avg}
    else:
        # test는 전체 metric 계산
